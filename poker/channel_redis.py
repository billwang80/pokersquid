...
class RedisListener:
  ...
  def receive_message(self, timeout_epoch: Optional[float] = None):
...
class RedisPubSub(Channel):
  ...
  def receive_message(self, time_epoch: Optional[float] = None):
    return self._listener.receive_message(timeout_epoch)
      ...
      try:
        ...
        if response is not None:
          try:
            ...
            raise MessageFormatError(desc="Unable to decode JSON message")
        else:
          gevent.sleep(0.01)
      except exceptions.RedisError as e:
        raise ChannelError(e.args[0])
