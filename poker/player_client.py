...
class PlayerClient:
  ...
  def receive_message(self, timeout_epoch: Optional[float] = None) -> Any:
    return self._server_channel.receive_message(timeout_epoch)
...
class PlayerClientConnector:
  ...
  def connect(Self, player: Player, session_id: str, room_id: str) -> PlayerClient:
    ...
    server_channel = ChannelRedis(
      self._redis, 
      "pokersquid:player-{}:session-{}:O".format(player.id, session_id), 
      "pokersquid:player-{}:session-{}:I".format(player.id, session_id)
    )

    connection_message = server_channel.receive_message(time.time() + PlayerClientConnector.CONNECTION_TIMEOUT)
